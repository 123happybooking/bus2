    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4><i class="bi bi-geo-alt"></i> 場所施設マスター</h4>
            ...
            <div class="bg-light p-2 mb-2 rounded" style="background-color: #F3F4F6 !important; border: 1px solid #E5E7EB;">
                <form method="GET" action="{{ route('masters.locations.index') }}" class="row g-2">
                    ...
                    <div class="col-auto">
                        <a href="{{ route('masters.locations.index') }}" class="btn btn-sm btn-outline-secondary px-3" 
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <form action="{{ route('masters.locations.destroy', $location) }}" method="POST" 
                                              class="d-inline" 
                                              onsubmit="return confirmDelete('{{ $location->name }}')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <td colspan="7" class="text-center py-4">
                                    @if(request('search'))
                                        <div class="text-muted">
                                            ...
                                            <p class="mb-0">検索条件に一致する場所施設が見つかりませんでした</p>
                                            <p class="small">検索キーワードを変更してお試しください</p>
                                        </div>
                                    @else
                                        <div class="text-muted">
                                            <i class="bi bi-geo-alt display-6 mb-2"></i>
                                            <p class="mb-0">場所施設データが登録されていません</p>
                                            <p class="small">「新規追加」ボタンから最初の場所施設を登録してください</p>
                                        </div>
                                    @endif
                                </td>
                            </tr>
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination pagination-sm mb-0">
                                    <li class="page-item {{ $locations->onFirstPage() ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $locations->previousPageUrl() }}" aria-label="Previous" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
                                    ...
                                    @if($start > 1)
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $locations->url(1) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">1</a>
                                        </li>
                                        @if($start > 2)
                                    ...
                                    @for($i = $start; $i <= $end; $i++)
                                        <li class="page-item {{ $i == $current ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $locations->url($i) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if($end < $last)
                                        ...
                                        @endif
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $locations->url($last) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $last }}</a>
                                        </li>
                                    @endif
                
                                    <li class="page-item {{ !$locations->hasMorePages() ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $locations->nextPageUrl() }}" aria-label="Next" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
