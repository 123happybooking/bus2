...
@section('title', '点検項目マスタ')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4><i class="bi bi-clipboard-check"></i> 点検項目マスタ</h4>
        <a href="{{ route('masters.driver-vehicle-check-items.create') }}" class="btn btn-primary">
    ...
    <div class="bg-light p-2 mb-2 rounded" style="background-color: #F3F4F6 !important; border: 1px solid #E5E7EB;">
        <form method="GET" action="{{ route('masters.driver-vehicle-check-items.index') }}" class="row g-2">
            <div class="col">
                <input type="text" name="search" class="form-control form-control-sm" style="border-color: #E5E7EB;" 
                       placeholder="カテゴリー、点検項目名で検索"
            ...
            <div class="col-auto">
                <a href="{{ route('masters.driver-vehicle-check-items.index') }}" class="btn btn-sm btn-outline-secondary px-3" 
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex gap-1 justify-content-center">
                            <a href="{{ route('masters.driver-vehicle-check-items.edit', $item) }}" 
                            ...
                            <form action="{{ route('masters.driver-vehicle-check-items.destroy', $item) }}" method="POST" 
                                  class="d-inline" onsubmit="return confirmDelete('{{ $item->item_name }}')">
                                @csrf
                                @method('DELETE')
                    ...
                    <td colspan="6" class="text-center py-4">
                        @if(request('search'))
                            <div class="text-muted">
                                ...
                                <p class="mb-0">検索条件に一致する点検項目が見つかりませんでした</p>
                            </div>
                        @else
                            <div class="text-muted">
                                <i class="bi bi-clipboard-check display-6 mb-2"></i>
                                <p class="mb-0">点検項目データが登録されていません</p>
                            </div>
                        @endif
                    </td>
                </tr>
    ...
    @if($checkItems->hasPages() || $checkItems->total() > 0)
        <div class="mt-3">
            <div class="d-flex flex-wrap justify-content-center align-items-center gap-2">
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination pagination-sm mb-0">
                        <li class="page-item {{ $checkItems->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $checkItems->previousPageUrl() }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">&laquo;</a>
                        </li>

                        @php
                            $current = $checkItems->currentPage();
                            $last = $checkItems->lastPage();
                            $start = max(1, $current - 2);
                            $end = min($last, $current + 2);
                        @endphp

                        @if($start > 1)
                            <li class="page-item"><a class="page-link" href="{{ $checkItems->url(1) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">1</a></li>
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            <li class="page-item {{ $i == $current ? 'active' : '' }}">
                                <a class="page-link" href="{{ $checkItems->url($i) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $i }}</a>
                            </li>
                        @endfor
                        ...
                        @if($end < $last)
                            ...
                            <li class="page-item"><a class="page-link" href="{{ $checkItems->url($last) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $last }}</a></li>
                        @endif

                        <li class="page-item {{ !$checkItems->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $checkItems->nextPageUrl() }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">&raquo;</a>
                        </li>
                    </ul>
                </nav>
            </div>
            <div class="text-center text-muted mt-2" style="font-size: 0.75rem;">
                表示中：{{ $checkItems->firstItem() ?? 0 }} - {{ $checkItems->lastItem() ?? 0 }} / 全 {{ $checkItems->total() }} 件
            </div>
        </div>
    @endif
</div>
