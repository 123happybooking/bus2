                ...
                <div class="d-flex gap-1"> <!-- gap-2 -> gap-1 -->
                    ...
                    @if($invoice->total_amount > $invoice->paid_amount && $invoice->type==1)
                        <button type="button" 
                                ...
                                data-request-amount="{{ number_format($invoice->total_amount, 0, '.', '') }}" 
                                data-balance-amount="{{ number_format($invoice->total_amount - $invoice->paid_amount, 0, '.', '') }}"
                                    ...
                                    @if(count($oldItems) > 0)
                                        @foreach($oldItems as $index => $oldItem)
                                            ...
                                            <tr data-index="{{ $index }}" draggable="true">
                                                ...
                                                <td class="py-1"><input type="number" class="form-control form-control-sm unit-price" name="items[{{ $index }}][unit_price]" value="{{ number_format($oldItem['unit_price'], 0, '.', '') }}" min="0" step="0.01"></td>
                    ...
                    <table class="table table-bordered table-hover align-middle bg-light opacity-75">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="text-center" style="width: 120px;">操作日</th>
