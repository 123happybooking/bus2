    ...
    <div class="modal fade" id="cashEntryModal" tabindex="-1" aria-hidden="true">
        <!-- 1. 表单地址 -->
        <form id="cashEntryForm" action="{{ route('masters.account-deposit.journal.store') }}" method="POST" novalidate>
            @csrf
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body pt-4 pb-3">
                        ...
                        <div class="row g-2 align-items-center">
                            <label class="col-3 col-form-label text-end fw-bold small" style="margin-top:-20px;">預金</label>
                            <div class="col-9 position-relative">
                            </div>
                            <label class="col-3 col-form-label text-end fw-bold small">補助科目</label>
                            <div class="col-9 position-relative">
                                <select class="form-control form-control-sm " name="deposit_sub_account_id" required>
                                    @foreach($subAccounts as $sub)
                                        <option value="{{ $sub->id }}">{{ $sub->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <hr style="margin-block: 0.3rem;">
                            <!-- 取引日 -->
