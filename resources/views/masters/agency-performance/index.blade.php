...
<div class="container-fluid px-4 py-0">
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h5 class="mb-0" style="color: #374151; font-size: 1.25rem;">送客実績</h5>
    </div>
    
    <div class="bg-light p-2 mb-2 rounded" style="background-color: #F3F4F6 !important; border: 1px solid #E5E7EB;">
        <form method="GET" action="{{ route('masters.agency-performance.index') }}" class="row g-1" id="searchForm">
            <div class="col-12">
                <div class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <div class="agency-dropdown">
                            <div class="dropdown">
                                <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" style="min-width: 140px; text-align: left; background-color: #fff; border-color: #ced4da;">
                                    <span id="agencySelectedText">選択</span>
                                    <span id="agencySelectedCount" class="selected-count" style="display: none;">0</span>
                                </button>
                                <div class="dropdown-menu p-0" style="min-width: 200px;">
                                    <div class="dropdown-header border-bottom px-3 py-2">
                                        <label class="d-flex align-items-center w-100" style="cursor: pointer;">
                                            <input type="checkbox" id="agencySelectAll" class="me-2"> 
                                            <span>全て選択</span>
                                        </label>
                                    </div>
                                    <div style="max-height: 250px; overflow-y: auto;">
                                        @foreach($allAgencies ?? [] as $agency)
                                            <label class="dropdown-item d-flex align-items-center" style="cursor: pointer;">
                                                <input type="checkbox" name="agency_checkbox" value="{{ $agency->id }}" class="me-2 agency-checkbox"
                                                    {{ in_array($agency->id, (array)$agencyIds) ? 'checked' : '' }}>
                                                {{ $agency->agency_name }}
                                                @if($agency->country || $agency->type)
                                                    ({{ $agency->country }}{{ $agency->country && $agency->type ? '、' : '' }}{{ $agency->type }})
                                                @endif
                                            </label>
                                        @endforeach
                                    </div>
                    ...
                    <div class="d-flex gap-1">
                        ...
                        <a href="{{ route('masters.agency-performance.index', ['reset_search' => 1]) }}" class="btn btn-sm btn-outline-secondary px-3"
    ...
    <div class="table-responsive" style="overflow-x: auto;">
        <table class="table table-bordered table-sm performance-table" style="font-size: 0.75rem;">
            <thead>
                <tr>
                    <th class="text-center" style="position: sticky; left: 0; background-color: #f8f9fa; z-index: 11; min-width: 120px; vertical-align: middle;">代理店</th>
                    @foreach($dates as $date)
                        <th class="text-center date-header" style="background-color: #e9ecef; min-width: 80px;">
                            {{ $date['display'] }}
                        </th>
                    @endforeach
                    <th class="text-center" style="background-color: #e9ecef; min-width: 100px;">合計</th>
                 </thead>
            </thead>
            <tbody>
                    ...
                    <tr style="background-color: {{ $rowBgColor }};">
                        ...
                        @foreach($dates as $date)
                            @php
                                ...
                                $stat = $statistics[$agencyId][$dateStr] ?? ['reservations' => 0, 'points' => 0];
                                $points = $stat['points'];
                                $formattedPoints = is_numeric($points) && floor($points) == $points ? (int)$points : $points;
                            @endphp
                        ...
                        <td class="text-center" style="background-color: {{ $rowBgColor }};">
                            @php
                                $agencyTotal = $agencyTotals[$agencyId] ?? ['reservations' => 0, 'points' => 0];
                                $totalPoints = $agencyTotal['points'];
                                $formattedTotalPoints = is_numeric($totalPoints) && floor($totalPoints) == $totalPoints ? (int)$totalPoints : $totalPoints;
                            @endphp
                            @if($agencyTotal['reservations'] != 0)
                                {{ $agencyTotal['reservations'] }}
                            @endif
                            <br>
                            @if($formattedTotalPoints != 0)
            ...
            <tfoot>
                ...
                <tr style="background-color: {{ $footBgColor }}; font-weight: bold;">
                    ...
                    @foreach($dates as $date)
                        @php
                            $dateStr = $date['date_str'];
                            $totalStat = $totalStatistics[$dateStr] ?? ['reservations' => 0, 'points' => 0];
                            $points = $totalStat['points'];
                            $formattedPoints = is_numeric($points) && floor($points) == $points ? (int)$points : $points;
                        @endphp
                        <td class="text-center" style="background-color: {{ $footBgColor }};">
                            @if($totalStat['reservations'] != 0)
                                {{ $totalStat['reservations'] }}
                            @endif
                            <br>
                            @if($formattedPoints != 0)
                                {{ $formattedPoints }}
                    ...
                    @endforeach
                    <td class="text-center" style="background-color: {{ $footBgColor }};">
                        @if($grandTotalReservations != 0)
                            {{ $grandTotalReservations }}
                        @endif
                        <br>
                        @php
                            $formattedGrandPoints = is_numeric($grandTotalPoints) && floor($grandTotalPoints) == $grandTotalPoints ? (int)$grandTotalPoints : $grandTotalPoints;
                        @endphp
                        @if($formattedGrandPoints != 0)
            ...
            <div style="padding: 20px;">
                ...
                <div class="mb-3">
                    <label class="d-flex align-items-center" style="cursor: pointer;">
                        <input type="checkbox" id="export_reservations" value="reservations" class="me-2" checked>
                        <span>予約数</span>
                    </label>
                </div>
                <div class="mb-3">
                    <label class="d-flex align-items-center" style="cursor: pointer;">
                        <input type="checkbox" id="export_points" value="points" class="me-2" checked>
                        <span>点数</span>
                    </label>
                </div>
...
<form id="exportForm" method="POST" action="{{ route('masters.agency-performance.export') }}" style="display: none;">
    @csrf
    <input type="hidden" name="start_date" id="export_start_date">
    <input type="hidden" name="end_date" id="export_end_date">
    <input type="hidden" name="agency_ids" id="export_agency_ids">
...
.agency-dropdown .dropdown-toggle:after {
    float: right;
    margin-top: 8px;
}

.agency-dropdown .dropdown-menu {
    min-width: 220px;
}

.agency-dropdown .dropdown-item {
    cursor: pointer;
}
...
function initAgencySelect() {
    ...
    const selectAllCheckbox = document.getElementById('agencySelectAll');
    const agencySelectedText = document.getElementById('agencySelectedText');
    const agencySelectedCount = document.getElementById('agencySelectedCount');
    ...
    function updateAgencyDisplay() {
        ...
        if (count === 0) {
            agencySelectedText.textContent = '代理店';
            agencySelectedCount.style.display = 'none';
        } else {
            agencySelectedText.textContent = '代理店';
            agencySelectedCount.textContent = count;
            agencySelectedCount.style.display = 'inline-block';
        }
        ...
        document.querySelectorAll('.agency-hidden-input').forEach(input => input.remove());
        
        selected.forEach(cb => {
            const hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.name = 'agency_ids[]';
            hiddenInput.value = cb.value;
            hiddenInput.className = 'agency-hidden-input';
    ...
    document.querySelectorAll('.agency-dropdown .dropdown-item').forEach(item => {
...
function exportExcel() {
    const reservationsChecked = document.getElementById('export_reservations').checked;
    const pointsChecked = document.getElementById('export_points').checked;
    
    if (!reservationsChecked && !pointsChecked) {
        alert('出力する項目を少なくとも1つ選択してください。');
        return;
    }
    
    const exportOptions = [];
    if (reservationsChecked) exportOptions.push('reservations');
    if (pointsChecked) exportOptions.push('points');
    ...
    const agencyCheckboxes = document.querySelectorAll('.agency-checkbox:checked');
    const agencyIds = Array.from(agencyCheckboxes).map(cb => cb.value);
    
    document.getElementById('export_start_date').value = startDate;
    document.getElementById('export_end_date').value = endDate;
    document.getElementById('export_agency_ids').value = JSON.stringify(agencyIds);
