    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4><i class="bi bi-gear"></i> 操作ステータス</h4>
                <a href="{{ route('masters.driver-operation-status.create') }}" class="btn btn-primary">
            ...
            @if(request('search'))
                <div class="alert alert-info mb-3">
                    ...
                    検索条件: "{{ request('search') }}" 
                    @if($operationStatuses->count() > 0)
                        - {{ $operationStatuses->total() }}件の結果が見つかりました
                    @else
                        - 該当する操作ステータスが見つかりませんでした
                    @endif
                </div>
            @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('masters.driver-operation-status.edit', $status) }}" 
                                        ...
                                        <form action="{{ route('masters.driver-operation-status.destroy', $status) }}" method="POST" 
                                              class="d-inline" 
                                              onsubmit="return confirmDelete('{{ $status->name }}')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <td colspan="5" class="text-center py-4">
                                    @if(request('search'))
                                        <div class="text-muted">
                                            ...
                                            <p class="mb-0">検索条件に一致する操作ステータスが見つかりませんでした</p>
                                            <p class="small">検索キーワードを変更してお試しください</p>
                                        </div>
                                    @else
                                        <div class="text-muted">
                                            <i class="bi bi-gear display-6 mb-2"></i>
                                            <p class="mb-0">操作ステータスデータが登録されていません</p>
                                            <p class="small">「新規追加」ボタンから最初の操作ステータスを登録してください</p>
                                        </div>
                                    @endif
                                </td>
                            </tr>
