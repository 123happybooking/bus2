...
<div class="container-fluid">
    ...
    <div class="bg-light p-2 mb-2 rounded" style="background-color: #F3F4F6 !important; border: 1px solid #E5E7EB;">
        <form method="GET" action="{{ route('masters.branches.index') }}" class="row g-2">
            <div class="col">
                <input type="text" name="search" class="form-control form-control-sm" style="border-color: #E5E7EB;" placeholder="営業所名、住所、電話番号で検索"
            ...
            <div class="col-auto">
                <a href="{{ route('masters.branches.index') }}" class="btn btn-sm btn-outline-secondary px-3" 
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination pagination-sm mb-0">
                        <li class="page-item {{ $branches->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $branches->previousPageUrl() }}" aria-label="Previous" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
                        ...
                        @if($start > 1)
                            <li class="page-item">
                                <a class="page-link" href="{{ $branches->url(1) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">1</a>
                            </li>
                            @if($start > 2)
                        ...
                        @for($i = $start; $i <= $end; $i++)
                            <li class="page-item {{ $i == $current ? 'active' : '' }}">
                                <a class="page-link" href="{{ $branches->url($i) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $i }}</a>
                            </li>
                        @endfor
                        ...
                        @if($end < $last)
                            ...
                            @endif
                            <li class="page-item">
                                <a class="page-link" href="{{ $branches->url($last) }}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $last }}</a>
                            </li>
                        @endif
    
                        <li class="page-item {{ !$branches->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $branches->nextPageUrl() }}" aria-label="Next" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">
