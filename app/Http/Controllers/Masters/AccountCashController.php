...
class AccountCashController extends Controller
{
    public function index(Request $request)
    {
        ...
        $account = Account::where("id", $accountConfig->account_cash_id)->first();
        ...
        return view('masters.account-cash.index', compact(
            'datas','periods','period_id','yearmonth','months','accounts','partners','taxes'
        ));
    }
    ...
    public function storeJournal(Request $request){ 
        ...
        try {
            $accountConfig = AccountConfig::first();
            $cashAccount = Account::where("id", $accountConfig->account_cash_id)->first();
            ...
            return redirect()->route('masters.account-cash.index')
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            return redirect()
                ->route('masters.account-cash.index')
                ->with([
                    'error' => '削除に失敗しました。',
                    'alert-type' => 'danger'
                ]);
    ...
    public function generatePdf()
    {
        ...
        //return view('masters.account-cash.pdf', $datas);
        try {
            // 1. 渲染 HTML
            $html = View::make('masters.account-cash.pdf', $datas)->render();
